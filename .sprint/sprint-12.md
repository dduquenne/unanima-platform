...
- [ ] `NEXT_PUBLIC_SIMULATION_MODE=true` reconnu par l'app
- [ ] Fixtures couvrent les 3 rôles et toutes les entités
- [ ] Login contourné en mode simulation avec choix du rôle
...
- [ ] Tous les écrans bénéficiaire s'affichent avec données fictives
- [ ] Tous les écrans consultant s'affichent avec données fictives
- [ ] Tous les écrans admin s'affichent avec données fictives
...
- [ ] Bandeau visible sur tous les écrans en mode simulation
- [ ] Sélecteur de rôle permet de basculer sans recharger
...
- [ ] `pnpm build --filter=@unanima/links` passe (simulation ON et OFF)
...
- [ ] `isSimulationMode()` retourne `true` quand `NEXT_PUBLIC_SIMULATION_MODE=true`
- [ ] `isSimulationMode()` retourne `false` par défaut (variable absente ou `false`)
- [ ] Utilisable côté client ET serveur (route handlers)
- [ ] Aucun impact quand le mode est désactivé
...
#### Critères d'acceptation
- [ ] Données typées avec les interfaces existantes (`Profile`, `PhaseValidation`, `Session`, etc.)
- [ ] UUIDs déterministes (pas de `crypto.randomUUID()`)
- [ ] Données réalistes (noms français, dates cohérentes, progression logique)
- [ ] Couverture de tous les états : phases libre/en_cours/validee, sessions passées/futures
- [ ] Au moins 1 bénéficiaire avec bilan terminé, 1 en cours, 1 nouveau
...
#### Critères d'acceptation
- [ ] En mode simulation, aucun appel à Supabase Auth
- [ ] Le rôle par défaut est `beneficiaire`
- [ ] Le rôle peut être changé via le sélecteur (cookie `simulation-role`)
- [ ] Les redirections par rôle fonctionnent (bénéficiaire → /dashboard, consultant → /beneficiaires, admin → /admin)
- [ ] En mode normal, aucun changement de comportement
...
#### Critères d'acceptation
- [ ] Dashboard bénéficiaire affiche progression, phases, sessions
- [ ] Page bilans affiche les questionnaires avec réponses pré-remplies
- [ ] Autosave retourne succès sans erreur
- [ ] Documents listés (téléchargement désactivé en simulation)
- [ ] Code existant inchangé quand simulation OFF
...
#### Critères d'acceptation
- [ ] Dashboard consultant affiche KPIs et liste bénéficiaires
- [ ] Dossier bénéficiaire détail affiche phases, sessions, notes
- [ ] Écritures (sessions, notes) retournent succès sans erreur
- [ ] Code existant inchangé quand simulation OFF
...
#### Critères d'acceptation
- [ ] Dashboard admin affiche 4 KPIs et tableau bénéficiaires
- [ ] Page utilisateurs affiche la liste avec recherche/filtres
- [ ] CRUD utilisateurs retourne succès sans erreur
- [ ] Code existant inchangé quand simulation OFF
...
#### Critères d'acceptation
- [ ] Bandeau visible sur toutes les pages protégées
- [ ] Sélecteur permet de changer de rôle
- [ ] Le changement redirige vers la page d'accueil du rôle
- [ ] Bandeau absent quand simulation OFF
- [ ] Accessible (contraste, focus, aria-label)
...
#### Critères d'acceptation
- [ ] Instructions claires pour activer/désactiver le mode
- [ ] Liste des limitations (pas de persistance, pas de téléchargement documents)
- [ ] Description des données fictives disponibles
...
#### Critères d'acceptation
- [ ] `isSimulationMode()` testé (true/false/absent)
- [ ] Fixtures valident les types TypeScript
- [ ] Fixtures contiennent les données attendues (nb profils, phases, etc.)
- [ ] Build passe avec simulation ON et OFF
- [ ] Aucune régression des tests existants
