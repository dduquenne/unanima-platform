---
title: "Guide de configuration et déploiement Vercel — CREAI Île-de-France"
id: GUIDE-DEPLOY-vercel-creai
version: 1.0.0
status: draft
author: Documentalix
created: 2026-03-22
updated: 2026-03-22
reviewers: []
tags: [vercel, deploiement, creai, production, configuration]
related: [deployment-checklist]
---

# Guide de configuration et déploiement Vercel — CREAI Île-de-France
...
- [ ] Un projet Supabase dédié à CREAI (ne pas réutiliser celui de Links ou Omega)
...
> **Important :** Le Root Directory doit pointer vers `apps/creai`, pas vers la racine du monorepo.

### 2.2 Vérifier la configuration `vercel.json`

Le fichier `apps/creai/vercel.json` est déjà configuré :
...
- **buildCommand** : Remonte à la racine du monorepo puis lance le build via Turborepo, filtré sur CREAI uniquement
...
|---|---|---|---|
| `NEXT_PUBLIC_SUPABASE_URL` | Plain text | Production, Preview, Development | URL du projet Supabase CREAI (ex: `https://xxxxx.supabase.co`) |
...
|---|---|---|
| `EMAIL_FROM` | Plain text | Adresse expéditeur des e-mails (ex: `noreply@creai-idf.fr`) |

### 3.4 Vérification

Utiliser le fichier de référence `apps/creai/.env.local.example` pour vérifier que toutes les variables sont configurées :
...
CREAI doit avoir son **propre projet Supabase**, distinct de Links et Omega. Les clés ne sont jamais partagées entre projets.

> **Données sensibles :** CREAI manipule des données médico-sociales. La sécurité du projet Supabase est critique. Activer l'authentification multi-facteurs (MFA) pour les comptes administrateurs.
...
**Tables métier CREAI :**
- `etablissements` — Établissements médico-sociaux suivis
- `diagnostics` — Diagnostics de transformation de l'offre
- `indicateurs` — Indicateurs de suivi
- `recommandations` — Recommandations issues des diagnostics
- `rapports` — Rapports de synthèse

### 4.3 Row Level Security (RLS)

Les politiques RLS doivent être activées et configurées pour les rôles CREAI :

| Rôle | Permissions |
|---|---|
| `admin_creai` | Accès total à tous les établissements et diagnostics |
| `direction` | Lecture de tous les établissements, écriture des recommandations |
| `coordonnateur` | Lecture/écriture des établissements assignés |
| `professionnel` | Lecture des diagnostics de ses établissements |
...
- [ ] Les données médico-sociales sont protégées par des politiques RLS strictes
...
2. Ajouter le domaine souhaité (ex: `appui.creai-idf.fr`)
3. Configurer les enregistrements DNS chez le registrar :

| Type | Nom | Valeur |
|---|---|---|
| CNAME | `appui` | `cname.vercel-dns.com` |
...
Le build CREAI suit ce pipeline (défini dans `turbo.json`) :

```
@unanima/core (build)
@unanima/auth (build)        ─┐
@unanima/db (build)           │
@unanima/dashboard (build)    ├─► @unanima/creai (build) ─► .next/
@unanima/email (build)        │
@unanima/rgpd (build)        ─┘
```
...
Le fichier `apps/creai/next.config.js` configure :
...
```bash
# Build isolé de CREAI (depuis la racine du monorepo)
pnpm build:creai

# Équivalent à :
pnpm turbo run build --filter=@unanima/creai
```
...
Le script `scripts/vercel-ignore.sh` est appelé automatiquement par Vercel avant chaque déploiement. Il vérifie si les fichiers pertinents à CREAI ont changé depuis le dernier déploiement.

### 7.2 Chemins surveillés

Le déploiement CREAI se déclenche **uniquement** si l'un de ces chemins a changé :

| Chemin | Raison |
|---|---|
| `apps/creai/` | Code de l'application |
...
Cela signifie qu'un commit modifiant uniquement `apps/links/` ne déclenchera **pas** de redéploiement de CREAI.
...
8. [ ] Création d'un établissement de test → succès
9. [ ] Suppression de l'établissement de test → succès
...
1. Dashboard Vercel → projet `unanima-creai`
...
- Les données médico-sociales étant sensibles, documenter tout incident et rollback dans le registre des traitements RGPD
...
**Cause :** Le build s'exécute depuis `apps/creai/` et ne trouve pas le lockfile.
...
**Cause :** Le script `vercel-ignore.sh` n'a détecté aucun changement pertinent à CREAI.
